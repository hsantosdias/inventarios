...
# Uptime
$lastBoot = if ($os.LastBootUpTime) { [Management.ManagementDateTimeConverter]::ToDateTime($os.LastBootUpTime) } else { $null }
...
$null = $sb.AppendLine("Instalação SO: $($os.InstallDate)")
$null = $sb.AppendLine("Última inicialização: $lastBoot  | Uptime: $($uptime.Days)d $($uptime.Hours)h $($uptime.Minutes)m")
$null = $sb.AppendLine("Domínio: $dominio  | Workgroup: $workgroup")
$null = $sb.AppendLine("BIOS: $($bios.SMBIOSBIOSVersion)  | Data BIOS: $($bios.ReleaseDate)")
$null = $sb.AppendLine("")
